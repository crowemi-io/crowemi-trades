...
type CorporateActionSyncTask struct {
...
func (t *CorporateActionSyncTask) Name() string {
	return "corporate_action_sync"
}

func (t *CorporateActionSyncTask) Schedule() string {
	return t.CronSchedule
}

func (t *CorporateActionSyncTask) Run(ctx context.Context) error {
	if t.Logger != nil {
	...
	for _, symbol := range symbols {
		...
		announcements, err := t.AlpacaClient.GetAnnouncements(alpaca.GetAnnouncementsRequest{
			CATypes: caTypes,
			Symbol:  symbol,
			Since:   since,
			Until:   until,
